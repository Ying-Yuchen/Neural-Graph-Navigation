...
if __name__ == "__main__":
    # Your edge list as tuples of (user_id, item_id)
    csv_file = '/mnt/nfs/yyc/0.datasets/amazon_small/clear_data/mini_graph/kcore_edge.csv'
    
    # Initialize the tokenizer
    tokenizer = GraphTokenizer(csv_file)
    
    # Perform random walks with a termination probability
    start_nodes = [0, 1, 2]  # Example start nodes
    length = 3   # 30% chance to terminate the walk at each step
    walks = tokenizer.random_walks(start_nodes, length)
    print("Random Walks:", walks)
    # Encode the walks into token sequences
    encoded_walks = tokenizer.encode_walks(walks)
    print("Encoded Walks:", encoded_walks)
    # Decode back to node sequences
    decoded_walks = tokenizer.decode_walks(encoded_walks)
    
    
    
    print("Decoded Walks:", decoded_walks)
