...
from llama.model import GraphDecoder
import random
import numpy as np
import dgl
import os
...
from llama.nx_utils import graph2path_v2
...
def load_amazon(path):
    ...
    # graph = dgl.graph((edge_index1, edge_index2))
...
def save_value2id(value_set, path, name):
    ...
    with open(file_path, 'w') as csvfile:
        ...
        writer.writerow(["Element", "ID"])  # 写入表头
        for element, idx in value2id.items():
            writer.writerow([element, idx])  # 写入每一对键值
...
def load_value2id(path, name=None):
    value2id = {}
    if name == None:
        dataset_list = ['HPRD', 'yeast', 'wordnet', 'dblp', 'human', 'amazon', 'lastfm', 'hamster', 'arxiv', 'nell', 'wikics']
...
def replace_and_select(lst, n, x, y):
    # 1. 从列表中随机选择 n 个不同的数字
    unique_numbers = list(set(lst))  # 去重
    if n > len(unique_numbers):
        # raise ValueError("n 不能大于列表中唯一数字的数量")
        n = len(unique_numbers)
    selected_numbers = random.sample(unique_numbers, n)  # 随机选择 n 个数字
...
def process_input_data(graph, graph_tokenizer:GraphTokenizer, start_nodes, graph_len, walk_len, epoch, params):
    ...
    ###课程学习，由易到男
    cur_mask_num = min(int(epoch / 2) + 1, params.dataprocess_config.walk_len)
    ...
    # ipdb.set_trace()
    ...
    elif params.dataprocess_config.data_obtain_method == 'hybrid':
        ...
        if i == 1: ###random_walk
            ...
            # valid_graphs = []
            valid_graphs = [walks[i, :graph_len[i]].tolist() for i in range(walks.size(0)) if -1 not in walks[i]]
        elif i == 2: ###neighborhood_sampling
            max_len = 0
            valid_graphs = []
            walks = graph_tokenizer.neighborhoods_sampling(start_nodes, params.dataprocess_config.neighbor_fanouts)
            for walk in walks:
    ...
    # valid_tokens_list = graph_tokenizer.encode_walks(valid_walks)
