...
from llama.encoders.nag_encoder import NAGEncoder
...
class NodeIdEmbedding(nn.Module):
    def __init__(self, d_model, max_len=512):
        ...
        # 注册为不可学习的缓冲区
        self.register_buffer('ne', ne)  # 形状 (1, max_len, d_model)
        self.num_embeddings = max_len  # 类似 nn.Embedding 的接口
        self.embedding_dim = d_model   # 类似 nn.Embedding 的接口
...
class Transformer(nn.Module):
    ...
    def forward(self, graph_features: torch.Tensor, tokens: torch.Tensor, subnode_ids: torch.Tensor, token_mask_len: torch.Tensor, start_pos: int):
        ...
        if tokens is not None:
            ...
            # print(h_tokens_subnode.shape)
            # print(h_tokens.shape)
        ...
        # print(positions_repeated.shape)
        # print(token_mask_len_uns.shape)
        valid_mask = positions_repeated < (token_mask_len_uns+1)  ##增加一个subgraph_feature的计数
